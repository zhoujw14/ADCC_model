...
for (i in 1:40) {
  #set up parameters
  parms <- c(g = 0.4, mu_SR = 4.3e-5, ds = 6.03, dr = 3.51, k=2.48, w=tmp.w, n=i)
  # Set up initial conditions
  if (i == 1) {
...
write.csv(con, file='con_w.csv', quote=FALSE, row.names=FALSE)
...
tmp.w <- fold[kk]*0.025
parms <- c(g = 0.4, mu_SR = 4.3e-5, mu_RS = 0.0014, ds = 6.03, dr = 3.51, k=2.48, w=tmp.w)
N <- 5000
f <- 0.239
# Set up empty data frame to store results
out <- data.frame()
...
write.csv(on3off3, file='on3off3_w.csv', quote=FALSE, row.names=FALSE)
p1 <- ggplot(con)+
  ...
  theme_classic()+
  scale_x_continuous(trans='log10', limits=c(0.01, 100))+scale_y_continuous(trans='log10')+
  labs(x='Fold on w',y='Total cell number', title='Resistance Evolution Rate w')
ggsave(p1, file='w_on3off3_con.tiff', width=5, height=2)
...
write.csv(on3off10, file='on3off10_w.csv', quote=FALSE, row.names=FALSE)
p2 <- ggplot(on3off3)+
  ...
  labs(x='Fold on w',y='Total cell number', title='Resistance Evolution Rate w')
ggsave(p2, file='w_on3off10_con.tiff', width=5, height=2)
...
write.csv(on3off5, file='on3off5_w.csv', quote=FALSE, row.names=FALSE)
p3 <- ggplot(on3off3)+
  geom_line(aes(x=fold_w, y=total, col='ON 3 days + OFF 3 days'), size=1.5)+
  ...
  scale_x_continuous(trans='log10')+scale_y_continuous(trans='log10')+
  labs(x='Fold on w',y='Total cell number', title='Resistance Evolution Rate w')
ggsave(p3, file='w_on3off5_con.tiff', width=5, height=2)


#plot difference
#w
con_w <- read.csv(file='con_w.csv',header=T)
on3off3_w <- read.csv(file='on3off3_w.csv',header=T)
diff_1 <- data.frame(fold = con_w$fold_w,
                     diff_w = con_w$total - on3off3_w$total)
ggplot(diff_1)+
  geom_line(aes(x=fold, y=diff_w), col='black', size=1.5)+
  theme_classic()+
  scale_x_continuous(trans='log10', limits=c(0.01, 10000))+
  scale_y_continuous(trans='identity')+
  labs(x='Fold on resistance evolution rate w',y='Total cell number difference', title='Continuous vs. ON 3 days + OFF 3 days')
