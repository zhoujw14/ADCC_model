...
for (kk in 1:length(fold)){
  ...
  for (i in 1:40) {
    #set up parameters
    parms <- c(g = 0.4, mu_SR = 0.347, ds = 5, dr = 4.26, k=2.67, w=tmp.w, n=i)
    # Set up initial conditions
    if (i == 1) {
...
for(kk in 1:length(fold)){
  ...
  tmp.w <- fold[kk]*0.028
  parms <- c(g = 0.4, mu_SR = 0.347, mu_RS = 0.0014, ds = 5, dr = 4.26, k=2.67, w=tmp.w)
  N <- 5000
  f <- 0.239
  # Set up empty data frame to store results
  out <- data.frame()
...
ggsave(p1, file='result/w_on3off3_con.tiff', width=5, height=2)
...
p2 <- ggplot(on3off3)+
  ...
  labs(x='Fold on w',y='Total cell number', title='Resistance Evolution Rate w')
ggsave(p2, file='result/w_on3off10_con.tiff', width=5, height=2)
...
p3 <- ggplot(on3off3)+
  ...
  scale_x_continuous(trans='log10')+scale_y_continuous(trans='log10')+
  labs(x='Fold on w',y='Total cell number', title='Resistance Evolution Rate w')
ggsave(p3, file='result/w_on3off5_con.tiff', width=5, height=2)
