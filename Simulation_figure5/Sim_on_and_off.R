...
results2 <- out %>%
  mutate(time2 = 6*(cycle-1)+time,
         N = S + R)
write.csv(results2, file='result/3on3off.csv', quote=FALSE, row.names=FALSE)
# for plot
#plot
...
ggsave(p1, file='result/on3off3.tiff', width=4, height=2)
...
write.csv(results2, file='result/3on5off.csv', quote=FALSE, row.names=FALSE)
#plot
...
ggsave(p2, file='result/on3off5.tiff', width=4, height=2)
...
write.csv(results2, file='result/3on10off.csv', quote=FALSE, row.names=FALSE)
#plot
...
ggsave(p3, file='result/on3off10.tiff', width=4, height=2)


#plot all scenarios
con <- read.csv(file='result/continuous.csv',header=T)
on3off3 <- read.csv(file='result/3on3off.csv',header=T)
on3off5 <- read.csv(file='result/3on5off.csv',header=T)
on3off10 <- read.csv(file='result/3on10off.csv',header=T)
...
p5 <- ggplot(con)+
  ...
  theme_classic()
ggsave(p5, file='result/total.tiff', width=8, height=3)
